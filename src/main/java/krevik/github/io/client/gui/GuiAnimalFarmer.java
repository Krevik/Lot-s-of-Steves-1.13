...
public class GuiAnimalFarmer extends Screen {

    private EntityAnimalFarmer farmer;
    private PlayerEntity player;
    private static ArrayList<Item> collectible_items;
    private static ArrayList<Item> breedable_items;
    private static ArrayList<Item> slashing_items;
    ...
    public GuiAnimalFarmer(PlayerEntity player, EntityAnimalFarmer farmer, int workSpeed, BlockPos workingRadius, ItemStack... stacks) {
    ...
    protected void init() {
        super.init();
        collectible_items = new ArrayList<>();
        breedable_items = new ArrayList<>();
        slashing_items = new ArrayList<>();
        ...
        int shiftZ = 24;
        //creates gui
        for(int i=0;i<collectible_items.size();i++){
            ToggleSwitchButton button = new ToggleSwitchButton(actualX+shiftX*i-shiftX*5*MathHelper.floor(i/5),actualY+shiftZ*MathHelper.floor(i/5),16,16, "button_farmer_gui_"+i,collectible_items.get(i));
            buttons.add(button);
            addButton(button);
        }

        actualX=(int) (width/2-24*2.5f);
        for(int i=0;i<breedable_items.size();i++){
            ToggleSwitchButton button = new ToggleSwitchButton(actualX+shiftX*i-shiftX*5*MathHelper.floor(i/5),actualY+shiftZ*MathHelper.floor(i/5),16,16, "button_farmer_gui_"+i,breedable_items.get(i));
            buttons.add(button);
            addButton(button);
        }

        actualX=(int) (width/2+64f);
        for(int i=0;i<slashing_items.size();i++){
            ToggleSwitchButton button = new ToggleSwitchButton(actualX+shiftX*i-shiftX*5*MathHelper.floor(i/5),actualY+shiftZ*MathHelper.floor(i/5),16,16, "button_farmer_gui_"+i,slashing_items.get(i));
            buttons.add(button);
            addButton(button);
        }
    ...
    private void loadAllowedItems(){
        collectible_items.add(Items.MILK_BUCKET);
        collectible_items.add(Items.EGG);

        //breedable items
        collectible_items.add(Items.WHEAT);

        //slasing items
        slashing_items.add(Items.PORKCHOP);
    }
