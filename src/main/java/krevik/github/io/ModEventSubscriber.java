...
public final class ModEventSubscriber {
    ...
    public static void onRegisterEntityTypes(final RegistryEvent.Register<EntityType<?>> event){
        event.getRegistry().registerAll(
                setup(ModEntities.ENTITY_AUTO_FARMER=EntityType.Builder.create(EntityAutoFarmer.class, EntityAutoFarmer::new).build(ModReference.MOD_ID+":"+"auto_farmer"),"auto_farmer"),
                setup(ModEntities.ENTITY_AUTO_LUMBERJACK=EntityType.Builder.create(EntityAutoLumberjack.class, EntityAutoLumberjack::new).build(ModReference.MOD_ID+":"+"auto_lumberjack"),"auto_lumberjack")
        );
    ...
    public static void onRegisterItems(final RegistryEvent.Register<Item> event){
        ...
        for (final Block block : ModUtil.getModEntries(ForgeRegistries.BLOCKS)) {
            registry.register(setup(new ItemBlock(block, new Item.Properties().group(ItemGroup.REDSTONE)), block.getRegistryName()));
        }
    }
